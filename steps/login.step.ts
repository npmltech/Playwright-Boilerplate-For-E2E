...
Then('eu devo ser logado com sucesso', async function (this: CustomWorld) {
    await expect(this.page).toHaveURL('https://testerbud.com/practice-login-form', { timeout: 10_000 });
});

Then('eu devo ver uma mensagem de erro', async function (this: CustomWorld) {
    await expect(this.page.locator('//*[contains(text(), "required")]')).toBeVisible({ timeout: 10_000 });
});