...
Runs Cucumber tests inside Docker with video recording enabled. Executes the test command explicitly with `docker compose run --rm ... sh -lc ...`. Videos and reports are saved to `./test-results/` and `./cucumber-reports/`.
